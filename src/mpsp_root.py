...
class MP:
    ...
    def formulate(self):
        ...
        routes = [np.array([0, k, prob.num_nodes - 1]) for k in range(1, prob.num_nodes - 1)]
        route_objs = [prob.l_CT[0, k] + prob.l_CT[k, prob.num_nodes - 1] for k in range(1, prob.num_nodes - 1)]
        drone_patterns = [[] for k in range(1, prob.num_nodes - 1)]
        drone_objs = [0 for k in range(1, prob.num_nodes - 1)]

        R = list(range(len(routes)))
        ...
        cpx.variables.add(
            names=[f'x_{r}' for r in R],
            obj=[float(route_objs[r] + drone_objs[r]) for r in R]
        )
        ...
        cpx.linear_constraints.add(
            lin_expr=[
                cplex.SparsePair(ind=[f'x_{r}' for r in R],
                                 val=[self.get_delta(j, routes[r]) + self.get_theta(j, drone_patterns[r]) for r in R])
                for j in prob.N
            ],
            rhs=[1] * len(prob.N),
            senses=['E'] * len(prob.N),
        ...
        self.routes = routes
        self.route_objs = route_objs
        self.drone_patterns = drone_patterns
        self.drone_objs = drone_objs
        self.R = R

        self.const_visit_all_nodes_start = const_visit_all_nodes_start
        self.const_visit_all_nodes_end = const_visit_all_nodes_end

    def get_delta(self, node, route):
        if node in route[1:-1]:
            return 1
        else:
            return 0

    def get_theta(self, node, drone_pattern):
        if node in [j for (i, j), d in drone_pattern]:
            return 1
        else:
            return 0
    ...
    def add_columns(self, cols):
        ...
        for col in cols:
            ...
            drone_obj = sum([prob.l_cD[i, j] for (i, j), d in drone_pattern])

            cpx.variables.add(
                obj=[float(truck_cost + drone_obj)],
                names=[f'x_{r}'],
                columns=[
                    cplex.SparsePair(
                        ind=[f'visit_{i}_at_least' for i in
                             list(truck_route[1:-1]) + [jj for (ii, jj), d in drone_pattern]],
                        val=[1 for i in list(truck_route[1:-1]) + [jj for (ii, jj), d in drone_pattern]]
                    )
                ]
            )
            ...
            self.drone_objs.append(drone_obj)
            self.R.append(r)
    ...
    def get_dual(self):
        ...
        alpha_ij = {(i, j): max(phi_i[j - 1] - self.prob.l_cD[i, j], 0) for i, j in self.prob.A_D}
    ...
    def nonzero_cols(self):
        return [val for val in self.sol.get_values(self.col_begin, self.cpx.variables.get_num() - 1) if
                not is_zero(val)]

    def get_solution(self):
        route_sol = [(self.sol.get_values(f'x_{r}'), list(self.routes[r]), list(self.drone_patterns[r])) for r in self.R
                     if self.sol.get_values(f'x_{r}') > 0.00001]
    ...
    def solve(self, phi_i, np_alpha_ij, Out_i, DroneOut_i, max_num_labels=-1, decomposition_level=1, ensure_exact_solving=True):
        ...
        def solve_sp1_sp2_parallelly(executor, phi_i, np_alpha_ij, Out_i, nb_DroneOut_i, sp2_check_num_col_found=True, aggressive_dominance=False, aggressive_extension=False):
            ...
            for future in as_completed(futures):
                ...
                # all_truck_route_list.extend(truck_route_list)
...
class BnBNode:
    ...
    def solve(self, incumbent=cplex.infinity, decomposition_level=1):
        ...
        while True:
            ...
            node_stats['cg_result'].append(tuple([node_stats['mp_time'] + node_stats['sp_time'],
                                                  node_stats['num_gen_cols'],
                                                  node_stats['num_labels'],
                                                  mp_obj_val
                                                  ]))
...
class BnPSolver:
    ...
    def solve(self, time_limit=3600, decomposition_level=1):
        ...
        while not nodes.empty() and elapsed_time <= time_limit:
            ...
            cg_result = node_stats['cg_result']
